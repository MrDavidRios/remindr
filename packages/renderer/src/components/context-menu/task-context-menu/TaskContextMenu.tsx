...
import { doIfTaskMenusAreClosed } from '/@/scripts/utils/menuutils';
...
export const TaskContextMenu: React.FC<TaskContextMenuProps> = ({
  ...
  function dropdownAction(actionTask: Task, action: (t: Task) => void) {
    ...
    const actionTaskSelected =
      selectedTasks.filter(t => t.creationTime === actionTask.creationTime).length > 0;
    ...
    >
      <ReactFocusLock>
        ...
        >
          ...
          <li
            className="menu-top-border hidden"
            title="Skip to next repeat date"
          >
            <img
              src={skipIcon}
              className="task-tile-image"
          ...
          >
            <img
              ...
              style={{
                filter:
                  'invert(9%) sepia(77%) saturate(6607%) hue-rotate(9deg) brightness(94%) contrast(116%)',
              }}
              draggable="false"
              alt=""
            />
