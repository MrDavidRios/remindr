...
describe('Action Bar', () => {
  const renderActionBar = ({ creating, onSave, taskOverrides }: RenderActionBarProps) => {
    renderWithProviders(
      <ActionBar task={{ ...testTask, ...taskOverrides }} creatingTask={creating} onSave={onSave} />,
      {},
    );
  };
  ...
  it('should show unpin button if task is pinned', async () => {
    renderActionBar({ creating: false, onSave: vi.fn(), taskOverrides: { pinned: true } });
  ...
  it('should show pin button if task is not pinned', async () => {
    renderActionBar({ creating: false, onSave: vi.fn(), taskOverrides: { pinned: false } });
