...
import {
  convertDOMRectToMenuRect,
  isMenuOpen,
} from "@renderer/scripts/utils/menuutils";
...
export const RemindersEditor: FC = () => {
  ...
  const currentlyEditedReminderActionType = useAppSelector(
    (state) => state.taskModificationState.reminderEditState.state
  );

  const [hoveringOverEditorHeader, setHoveringOverEditorHeader] =
    useState(false);
  ...
  const openReminderEditMenu = (
    ...
    const editorRect = convertDOMRectToMenuRect(
    ...
    dispatch(
      setReminderEditState({ idx, state: creatingReminder ? "create" : "edit" })
    );
    ...
    const reminderEditorHeaderOffset = !anchor
      ? (editorRect?.height ?? 0) - 26
      : 0;

    const taskModificationInterface = document.querySelector(
    ...
    dispatch(
      setFloatingMenuPosition({
        ...
        positionInfo: {
          ...
          yOffset: {
            topAnchored:
              scrollTopDiff + additionalOffset - reminderEditorHeaderOffset,
      ...
      >
        <button
          ...
          onClick={async () => {
            ...
            editedTaskClone.scheduledReminders.push(
              JSON.parse(JSON.stringify(newReminder))
            );
            const sortedReminders = sortReminders(
              editedTaskClone.scheduledReminders
            );
            const newReminderIdx = sortedReminders.findIndex(
              (r) => r.id === newReminderId
            );

            dispatch(
              setEditedTask({ creating: undefined, task: editedTaskClone })
            );
            ...
            const tileRect = convertDOMRectToMenuRect(
              lastReminderTile?.getBoundingClientRect()
            );
        ...
        {editedTaskClone.scheduledReminders.map((reminder, idx) => {
          return (
            <ReminderTile
              ...
              onEditReminder={(tileRect?: MenuRect) =>
                openReminderEditMenu(tileRect, idx)
              }
              onDeleteReminder={() => onDeleteReminder(idx)}
              selected={
                currentlyEditedReminderIdx === idx
                  ? { actionType: currentlyEditedReminderActionType }
                  : undefined
              }
            />
          );
