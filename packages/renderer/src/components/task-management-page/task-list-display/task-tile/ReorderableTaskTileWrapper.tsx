...
const taskSelectionChange = (
  task: Task,
  selectedTasksBefore: Task[],
  selectedTasksAfter: Task[]
) => {
  const wasSelectedBefore = _.some(
    selectedTasksBefore,
    (selectedTask) => selectedTask.creationTime === task.creationTime
  );
  const isSelectedAfter = _.some(
    selectedTasksAfter,
    (selectedTask) => selectedTask.creationTime === task.creationTime
  );
...
> = ({ task, onReorderComplete }) => {
  ...
  const selectedTasks = useAppSelector(
    ...
    (selectedTasksBefore, selectedTasksAfter) =>
      taskSelectionChange(task, selectedTasksBefore, selectedTasksAfter)
  );
  ...
  const taskTileProps = {
    ...
    onContextMenu: (e: React.MouseEvent<HTMLLIElement, MouseEvent>) =>
      dispatch(
        showContextMenu({
          contextMenu: ContextMenuType.TaskContextMenu,
          task,
          x: e.clientX,
          y: e.clientY,
        })
...
const taskTileClasses = (
  completed: boolean,
  hasIndicators: boolean,
  selected: boolean
) => {
  return `task-tile frosted ${hasIndicators ? "" : "task-tile-no-attributes"} ${
    selected ? "selected" : ""
  } ${completed ? "completed" : ""}`;
};
...
function handleTaskTileClick(
  ...
  const onlySelectedTaskIsCurrentTask =
    ...
    selectedTasks[0].creationTime === task.creationTime;
