...
export const TaskListDisplay: React.FC<TaskListDisplayProps> = ({
  ...
  const taskListGetStatus = useAppSelector(
    (state) => state.taskList.taskListGetStatus
  );
  const taskView = useAppSelector((state) => state.pageState.currentPage);
  const isInColumnView = taskView === Page.ColumnView;
  ...
  const taskListOutdated = useAppSelector(
    (state) => state.taskList.taskListDisplayOutdated
  );
  useEffect(() => {
    if (taskListOutdated) {
  ...
  return (
    <div id="listWrapper" className={taskMenuShown ? "show-task-menu" : ""}>
      ...
      <ul
        id="taskListDisplayContainer"
        className={`transition-disabled ${isInColumnView ? "column-view" : ""}`}
      >
...
function attemptGetTaskList(taskListGetStatus: string, dispatch: AppDispatch) {
  if (taskListGetStatus === "pending" || taskListGetStatus === "succeeded")
    return;
